...
// Tip: 주석은 여기서부터 아래쪽으로 달아야 한다.
// (ng generate 명령 시 에러가 발생함.)

// @NgModule 형태의 키워드는 자바에서 Annotation 이라고 부르던 문법.
// 여기서는 Decorator라고 부른다.
// 자바의 애노테이션이 클래스이듯이 앵귤러의 데코레이터는 함수이다.
// 즉 @NgModule({}) 문법은 NgModule 함수를 호출하면서
// 객체를 전달한다는 뜻이다.
// 객체의 내용으로 미리 정해진 프로퍼티키를 사용하여 값을 설정하면
// NgModule 함수가 받아서 AppModule 객체에 동적으로 코드를
// 추가하는 방식입니다.
...
/*
{
  ...
  bootstrap: [AppComponent] // 시작화면에서 사용할 화면기술(component)
}                           // 을 지정
*/
